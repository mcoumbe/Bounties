...
public class BountiesDao extends Dao {
	...
	public List<Bounty> getActiveBounties() {
		...
		sb.append(
				"SELECT b.id, b.player, sum(s.amount) AS amount FROM Wanted_Bounty b ")
	...
	public int saveDeath(String playerName, String cause) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("INSERT INTO Wanted_Death (player, cause)").append(
				" VALUES (?, ?)");
		String sql = sb.toString();
	...
	public void updateDeath(String killer, int deathId) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("UPDATE Wanted_Death").append(" SET killer = ?")
				.append(" WHERE id = ?");
		String sql = sb.toString();
	...
	public void endBounty(int bountyId) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("UPDATE Wanted_Bounty").append(" SET claimed_date = Now()")
				.append(" WHERE id = ?");
		String sql = sb.toString();
	...
	public void addKill(int bountyId, int deathId) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("INSERT INTO Wanted_Kill (death_id, bounty_id)").append(
				" VALUES (?, ?)");
		String sql = sb.toString();
	...
	public Bounty getBounty(String playerName) {
		...
		sb.append(
				...
				.append(" INNER JOIN Wanted_Sponsor s ON b.id = s.bounties_id")
				.append(" WHERE b.claimed_date IS NULL")
				.append(" AND b.player = ?").append(" GROUP BY b.id, b.player");
		String sql = sb.toString();
	...
	public int createBounty(String playerName) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append("INSERT INTO Wanted_Bounty (player)").append(" VALUES (?)");
		String sql = sb.toString();
	...
	public Bounty getBounty(int ranking) {
		...
		sb.append(
				...
				.append(" WHERE b.claimed_date IS NULL")
				.append(" GROUP BY b.id, b.player")
				.append(" ORDER BY amount DESC").append(" LIMIT ?, 1");
		String sql = sb.toString();
