...
public class BountiesCommandExecutor implements CommandExecutor {

	private Wanted plugin;

	public BountiesCommandExecutor(Wanted plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label,
		...
		if (cmd.getName().equalsIgnoreCase("bounty")) {
			sender.sendMessage(Bounties.getHelp());
			return true;
		}
		if (cmd.getName().equalsIgnoreCase("badd")) {
			...
			if (!errors.isEmpty()) {
				...
				sender.sendMessage(Bounties.BOUNTY_ADD_SUCCESS);
				sender.sendMessage((new StringBuilder(String
						.valueOf(Bounties.LOG_NAME))).append(
						"Bounty taxed at 5%").toString());
				plugin.getServer().broadcastMessage(
						Bounties.bountyAmount(args[0], plugin));
				return true;
			}
		}
		if (cmd.getName().equalsIgnoreCase("bdel")) {
